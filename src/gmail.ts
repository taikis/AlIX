...
export function sendMessage(to: string, subject: string, text: string) {
	createMail(subject, to, text).then((raw) => {
		// Load client secrets from a local file.
		fs.readFile("cred/credentials.json", (err: any, content: any) => {
 ...
 */
function authorize(credentials: any, callback: any, raw: string) {
 ...
 */
function getNewToken(oAuth2Client: any, callback: any, raw: string) {
 ...
 */
function sendGmail(auth: any, raw: string) {
...
async function createMail(subject: string, to: string, text: string) {
